    ...
    <div className="max-w-6xl mx-auto">
      ...
      <header className="flex justify-between items-center mb-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="bg-primary hover:bg-red-900 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all">
      ...
      <div className="mb-10">
        ...
        <p className="text-gray-400 flex items-center gap-2 text-sm">
          <span className="opacity-70">📅</span> Thursday, March 12
        </p>
      </div>
      ...
      <div className="flex justify-between items-end mb-6">
        ...
        <span className="text-primary text-xs cursor-pointer hover:underline font-medium">View All</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {['Habit Tracker', 'Todo List', 'Quick Notes', 'Diary'].map((item) => (
          <div key={item} className="h-48 bg-white/5 border border-white/5 rounded-2xl hover:border-white/10 transition-all cursor-pointer p-4 group">
             <div className="w-full h-3/4 bg-background/50 rounded-xl mb-3 group-hover:bg-background/80 transition-all"></div>
             <p className="text-sm font-medium">{item}</p>
             <p className="text-[10px] text-gray-500">Manage your daily {item.toLowerCase()}</p>
          </div>
        ))}
      </div>
    </div>
...
function StatCard({ icon, label, value, badge }: any) {
  return (
    <div className="bg-white/5 border border-white/5 p-6 rounded-2xl hover:bg-white/[0.07] transition-all">
      ...
      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.15em] mb-1">{label}</p>
